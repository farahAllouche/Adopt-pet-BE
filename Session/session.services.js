const { getUser, register,  login, logout, getUserInfo, updateUserInfo, intrestedIn} = require('./session.controller')
...
function registerHandler (req, res) {
    ...
    const userDetails = {
      ...
      register(userDetails).then(({ accessToken, refreshToken , userId}) => {
    ...
    .catch((err)=> {
        console.log(err);
            err.keyPattern?  res.status(409).send({message : "User already exixts ! "}) :res.status(400).send({message : 'Registration failed ! '})
...
function updateInfoUserHandler ( req, res) {
    ...
    if(userDetails.currentPwd) {
       ...
       getUserInfo(req.user.userId).then((user)=> {
        ...
        updateUserInfo(req.user.userId, userDetails).then((user)=> {
            res.send(user)
        })
        .catch((err)=> {
  ...
  module.exports = {getSessionHandler, loginHandler, registerHandler, logoutHandler, getUserInfoHandler, updateInfoUserHandler, updateUserInerest, getUserHandler}